...
func TestNewHTTPClient(t *testing.T) {
	...
	if client.Config.Server != config.Server {
		t.Errorf("Expected server %s, got %s", config.Server, client.Config.Server)
	}
...
func TestHTTPClientServerError(t *testing.T) {
	...
	// Should be a general error, not ExitError
	if _, ok := err.(*ExitError); ok {
		t.Error("Expected general error, not ExitError")
	}
}
...
func TestHTTPClientInvalidJSON(t *testing.T) {
	...
	// Should be a general error about decoding
	if exitErr, ok := err.(*ExitError); ok {
		t.Errorf("Expected general error, got ExitError with code %d", exitErr.Code)
	}
}
