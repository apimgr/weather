...
func (ts *TorService) Start(httpPort int) error {
	...
	conf := &tor.StartConf{
		DataDir:         ts.dataDir,            // Our isolated data directory
		NoHush:          false,                 // Show startup messages
		TempDataDirBase: filepath.Join(os.TempDir(), "weather-tor"),
		NoAutoSocksPort: false,                 // Let bine pick available ports (avoids system Tor 9050/9051)
