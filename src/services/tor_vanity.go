...
type VanityGenerator struct {
	...
	notifyCh chan string // Channel to notify when generation completes
}
...
func (vg *VanityGenerator) updateEstimate() {
	...
	if remainingAttempts < 0 {
		remainingAttempts = expectedAttempts * 0.1 // At least 10% more
	}
...
func EstimateTime(prefixLength int, attemptsPerSec float64) string {
	if attemptsPerSec == 0 {
		attemptsPerSec = 100000 // Default estimate: 100k attempts/sec
	}
