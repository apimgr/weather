...
func (m *SettingsModel) InitializeDefaults(backupPath ...string) error {
	...
	defaults := map[string]Setting{
		...
		"server.tagline":        {Value: "Your personal weather dashboard", Type: "string"},
		"server.description":    {Value: "A comprehensive platform for weather forecasts, moon phases, earthquakes, and hurricane tracking.", Type: "string"},
		"server.address":        {Value: "0.0.0.0", Type: "string"},
		"server.http_port":      {Value: "0", Type: "number"},
		"server.https_port":     {Value: "0", Type: "number"},
		"server.https_enabled":  {Value: "false", Type: "boolean"},
		"server.timezone":       {Value: "America/New_York", Type: "string"},
		"server.date_format":    {Value: "US", Type: "string"},
		"server.time_format":    {Value: "12-hour", Type: "string"},

		// Security settings
		"security.session_timeout":     {Value: "2592000", Type: "number"}, // 30 days
		"security.max_login_attempts":  {Value: "5", Type: "number"},
		"security.lockout_duration":    {Value: "30", Type: "number"},
		"security.password_min_length": {Value: "8", Type: "number"},

		// Features
		"features.registration_enabled": {Value: "true", Type: "boolean"},
		"features.api_enabled":          {Value: "true", Type: "boolean"},
		"features.weather_alerts":       {Value: "true", Type: "boolean"},
	...
	for key, setting := range defaults {
		...
		if err == sql.ErrNoRows {
			if err := m.Set(key, setting.Value, setting.Type); err != nil {
